...
class EloModel:
    ...
    def load_ratings_from_db(self):
        """Load team ratings from database instead of training."""
        with get_db_context() as db:
            teams = db.query(Team).filter(Team.elo_rating.isnot(None)).all()
            for team in teams:
                if team.elo_rating:
                    self.ratings[team.id] = team.elo_rating
            
            # If no ratings in DB, initialize with defaults
            if not self.ratings:
                self.initialize_ratings()
            
            logger.info(f"Loaded {len(self.ratings)} team ratings from database")
